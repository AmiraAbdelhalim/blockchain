...
class Blockchain:
    ...
    def proof_of_work(self, previous_proof):
        '''
        the problem miners have to solve it is hard to solve but easy to verify
        Parameters
        ----------
        previous_proof : TYPE
    ...
    def is_chain_valid(self, chain):
        ...
        while block_index < len(chain):
            ...
            hash_operation = hashlib.sha256(str(current_proof**2 - previous_proof**2)
                                            .encode()).hexdigest()
            if hash_operation[:4] != '0000':
                return False
...
# 5) check if blockchain is valid
@app.route('/check_blockchain_valid', methods=['GET'])
